...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80,
                 anchors= [[ 9,13,  22,20,  19,47 ], [ 48,34,  43,76,  90,68 ], [ 70,150,  151,132,  119,271 ], [ 218,307,  369,190,  462,396 ]],
                 # [[19, 27, 44, 40, 38, 94], [96, 68, 86, 152, 180, 137], [140, 301, 303, 264, 238, 542], [436, 615, 739, 380, 925, 792]],
                 ch=(), inplace=True):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = x[i].sigmoid()
                y[..., 0:2] = (y[..., 0:2].cuda() * 2. - 0.5 + self.grid[i].cuda())
                y[..., 0:2] = y[..., 0:2] * self.stride[i]  # xy
                y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                z.append(y.view(bs, -1, self.no))
...
class YOLOR(nn.Module):
    def __init__(self, params, ch=3, nc=None, anchors=None, verbose=False):  # model, input channels, number of classes
        super().__init__()
        with open(params.yaml, encoding='ascii', errors='ignore') as f:
...
def parse_model(params, d, ch, verbose=False):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in [Depth, Backbone, IDetect]:
...
def YOLOR_depth(params):
    params.yaml = 'models/yolor-p6-depth.yaml'
    return YOLOR(params)


def YOLOR_backbone(params):
